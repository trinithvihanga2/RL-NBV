...
{
    ChamferDistanceKernelLauncher(
        xyz1.size(0), xyz1.size(1), xyz1.data_ptr<float>(),  // ✅ Fixed
        xyz2.size(1), xyz2.data_ptr<float>(),                 // ✅ Fixed
        dist1.data_ptr<float>(), idx1.data_ptr<int>(),        // ✅ Fixed
        dist2.data_ptr<float>(), idx2.data_ptr<int>());       // ✅ Fixed
}
    ...
    at::Tensor idx2)
{
    ChamferDistanceGradKernelLauncher(
        xyz1.size(0), xyz1.size(1), xyz1.data_ptr<float>(),   // ✅ Fixed
        xyz2.size(1), xyz2.data_ptr<float>(),                  // ✅ Fixed
        graddist1.data_ptr<float>(), idx1.data_ptr<int>(),     // ✅ Fixed
        graddist2.data_ptr<float>(), idx2.data_ptr<int>(),     // ✅ Fixed
        gradxyz1.data_ptr<float>(), gradxyz2.data_ptr<float>()); // ✅ Fixed
}
    ...
    const at::Tensor idx2)
{
    ...
    const float* xyz1_data = xyz1.data_ptr<float>();  // ✅ Fixed
    const float* xyz2_data = xyz2.data_ptr<float>();  // ✅ Fixed
    float* dist1_data = dist1.data_ptr<float>();      // ✅ Fixed
    float* dist2_data = dist2.data_ptr<float>();      // ✅ Fixed
    int* idx1_data = idx1.data_ptr<int>();            // ✅ Fixed
    int* idx2_data = idx2.data_ptr<int>();            // ✅ Fixed
    ...
    at::Tensor idx2)
{
    ...
    const float* xyz1_data = xyz1.data_ptr<float>();        // ✅ Fixed
    const float* xyz2_data = xyz2.data_ptr<float>();        // ✅ Fixed
    float* gradxyz1_data = gradxyz1.data_ptr<float>();      // ✅ Fixed
    float* gradxyz2_data = gradxyz2.data_ptr<float>();      // ✅ Fixed
    const float* graddist1_data = graddist1.data_ptr<float>(); // ✅ Fixed
    const float* graddist2_data = graddist2.data_ptr<float>(); // ✅ Fixed
    const int* idx1_data = idx1.data_ptr<int>();            // ✅ Fixed
    const int* idx2_data = idx2.data_ptr<int>();            // ✅ Fixed

    for (int i = 0; i < b*n*3; i++) gradxyz1_data[i] = 0;
    for (int i = 0; i < b*m*3; i++) gradxyz2_data[i] = 0;
