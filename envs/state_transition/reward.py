...
def calculate_continuous_reward(
    ...
    if is_reward_with_cur_coverage:
        if step_cnt < 4:
            coverage_reward = cover_add * 100
        else:
            if cover_add <= 0:
                coverage_reward = cover_add * 100
            else:
                remain = 1.0 - (current_coverage - cover_add)
                coverage_reward = (cover_add / remain) * 50 + cover_add * 50
    elif is_ratio_reward:
        if cover_add <= 0:
            coverage_reward = cover_add * 100
        else:
            remain = 1.0 - (current_coverage - cover_add)
            coverage_reward = (cover_add / remain) * 100
    else:
        coverage_reward = cover_add * 100
    ...
    step_penalty = 1.0 # Flat penalty to heavily discourage 'do nothing' policies
