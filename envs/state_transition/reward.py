...
def calculate_continuous_reward(
    ...
    if is_reward_with_cur_coverage:
        if step_cnt < 4:
            coverage_reward = cover_add * 1000
        else:
            if cover_add <= 0:
                coverage_reward = cover_add * 1000
            else:
                remain = 1.0 - (current_coverage - cover_add)
                coverage_reward = (cover_add / remain) * 500 + cover_add * 500
    elif is_ratio_reward:
        if cover_add <= 0:
            coverage_reward = cover_add * 1000
        else:
            remain = 1.0 - (current_coverage - cover_add)
            coverage_reward = (cover_add / remain) * 1000
    else:
        coverage_reward = cover_add * 1000
    ...
    step_penalty = 10.0 # Flat penalty to heavily discourage 'do nothing' policies
    
    return float(coverage_reward - time_penalty - fuel_penalty - collision_penalty - step_penalty)
